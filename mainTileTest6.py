...
class Leader(object):
    ...
    '''
    def __init__(self, parent, portList):#, portList):
    ...
    def listenThread(self,port,n):
        ...
        while True:
            ...
            try:
                ...
                s.close()
                break
        self.listenThread(port,n)
        
    def listenThread_OLD(self,port,n):
        '''
        Listens on a port in it's own thread for new clients
        to connect
        '''
    ...
    def connect(self,timeOut):
        '''
        different style of TCP connection that may work
        with firewalls?
        '''
        timer = lilTimer()
        timer.tic()
        ...
        attempts = 0
        maxAttempts = 500
        while wait<timeOut and attempts<maxAttempts:
            try:
                HOST, PORT = self.host, self.port
                #self.conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                self.conn = socket.create_connection((HOST,PORT))
                print("Should be connected now...")
                self.f = self.conn.makefile("r+",bufsize=0)
    ...
    def waitForResponse(self,timeOut):
        """
        waits for any output for "timeOut" seconds
        
        bool,string = waitForResponse(timeOut)
        """
        sock = self.conn
        #received = recv_end(sock,self,0,1)
        received = self.getLastMessage()
        if received:
            try:
                num = float(received)
                return True,num
    ...
    def runCode(self,data,*args):
        if len(args)>0:
            response = args[0]
        else:
            response = False
        if True:#try:
            print("Message to %d:"%self.port)
            print(data)
            ...
            if response:
                ...
                return response
        if False:#except:
            print("unnable to connect to host")
            printError(sys.exc_info())
            print(data)
            return False,False
...
#        self.Bind(wx.EVT_MOVE,self.update)
    ...
    def setLeader(self, portList):
        '''
        Sets this process to be the controlling Leader
        '''
        self.runType = 'Leader'
        self.Com = Leader(self, portList)#, portList)
    ...
    def initializeDisplay(self):
        ...
        self.windowSizeText = wx_StaticText(panel,-1,'',(45,45))
        self.modelPosText = wx_StaticText(panel,-1,'',(45,65))
        self.modelAbsPosText = wx_StaticText(panel,-1,'',(45,85))
        self.MX = 0
        self.MY = 0
        self.newMx = 0
        self.newMy = 0
