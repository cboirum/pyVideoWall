...
v5 - added zoomCenter on mouse position, zoom is applied then picture vertically and 
    horizontally offset about the center by 1/2 the change in vertical and horizontal pixels,
    respectively.
...
#                                         wx.BitmapFromImage(img))
        ...
        self.minHeight = 100#self.parent.Parent.minZoom * self.bmp.GetHeight
        self.minWidth = 100#self.parent.Parent.minZoom * self.bmp.GetHeight
        
        self.CreateDisplay()
        
    def CreateDisplay(self):
        self.marker = wx.StaticBitmap(self.parent,wx.ID_ANY, wx.BitmapFromImage(self.data1))
    ...
    def MoveXY(self, x, y):
        '''
        Moves model to x and y in Window coordinates
        '''
        #self.display.Freeze()
        self.display.MoveXY(x, y)
        #self.marker.Set
    ...
    def Move2ScreenXY(self,x,y):
        '''
        Moves model to x, and y in screen coordinates
        '''
#        model2ScreenX = M2sX = self.parent.Parent.M2sX
#        model2ScreenY = M2sY = self.parent.Parent.M2sY
#        
#        model2Window = M2sX
    ...
    def SetDisplay(self):
        ...
        self.maxHeight = self.initHeight*self.parent.Parent.maxZoom 
        self.maxWidth = self.initWidth*self.parent.Parent.maxZoom
        self.display.SetBitmap(self.bmp)
        
    def Zoom(self,amount,center):
        '''
        Scales the bitmap to % of original size
        and maintains center by:
        
        zoomCenter on mouse position, zoom is applied then picture vertically and 
        horizontally offset about the center by 1/2 the change in vertical and horizontal pixels,
        respectively.
        '''
        ...
        model2Screen = self.parent.Parent.ClientToScreen((self.parent.Parent.MX,self.parent.Parent.MY))
        
        model2ScreenX = self.parent.Parent.M2sX
        model2ScreenY = self.parent.Parent.M2sY
        
        window2Screen = (self.parent.Parent.newX,
                         self.parent.Parent.newY)
        
        window2ScreenX = self.parent.Parent.newX
        window2ScreenY = self.parent.Parent.newY
        
        
        mouse2WindowX = mouse2ScreenX - window2ScreenX-9
        mouse2WindowY = mouse2ScreenY - window2ScreenY-36
        
        mouse2Window = (mouse2WindowX,
                        mouse2WindowY)
        
        model2WindowX = self.parent.Parent.MX
        model2WindowY = self.parent.Parent.MY
        
        model2MouseX = model2WindowX-mouse2WindowX#+9
        model2MouseY = model2WindowY-mouse2WindowY#+36
        
        model2Mouse = (model2MouseX,
                       model2MouseY)        
        
        CurrentHeight = self.bmp.GetHeight()
        CurrentWidth = self.bmp.GetWidth()
        
        sWidth = width + int(width * amount)
        sHeight = height + int(height * amount)
        
        stretchX = (CurrentHeight - sHeight)
        stretchY = (CurrentWidth - sWidth)
        
        newX = (stretchX/2 + model2WindowX)
        newY = (stretchY/2 + model2WindowY)
        newX = model2WindowX
        newY = model2WindowY
        
        model2ScreenX = newX + window2ScreenX
        model2ScreenY = newY + window2ScreenY
        
        model2Window = (model2WindowX,
                        model2WindowY)
        
        centerPointStart = (model2WindowX - model2Mouse[0],#(CurrentWidth/2),
                            model2WindowY - model2Mouse[1])#(CurrentHeight/2))
        centerPointEnd = (model2WindowX - model2Mouse[0]-(sWidth-CurrentWidth)/(CurrentWidth/(model2Mouse[0]*1.0)),
                          model2WindowY - model2Mouse[1]-(sHeight-CurrentHeight)/(CurrentHeight/(model2Mouse[1]*1.0)))
        
#        centerPointStart = (mouse2Window,
#                            mouse2Window)
#        centerPointEnd = (mouse2Window + (sWidth/2),
#                          mouse2Window + (sHeight/2))
        
        compensation = (centerPointEnd[0] - centerPointStart[0],
                        centerPointEnd[1] - centerPointStart[1])
        print"CenterStart :",centerPointStart
        print"CenterEnd   :",centerPointStart
        print"Compensation:",compensation
        print"% mouse position:",CurrentWidth/(model2Mouse[0]*1.0),CurrentHeight/(model2Mouse[1]*1.0)
        
        newModel2window = (model2Window[0] - compensation[0],
                           model2Window[1] - compensation[1])        
        
#        print"Height/Width:",CurrentHeight,CurrentWidth
#        print"Height/2",CurrentHeight/2
#        print"CenterPoint Position :",centerPointStart
#        print"Model2Window Position:",model2WindowX,model2WindowY
#        #print"Model2Screen Position:",model2Screen#model2ScreenX,model2ScreenY
#        #print"Mouse Position:",cX,cY
#        print"Model2mouseX/Y:",model2MouseX,model2MouseY
#        print"mouse2Window  :",mouse2Window
#        print"Mouse2Screen  :",center
#        print"Window2Screen :",window2Screen
#        #print"Mouse2Window  :",#self.parent.Parent.ClientToScreen(center)
#        #print"ModelDx/y:",stretchX,stretchY
#        print"newX/Y:",newX,newY
        ...
        dimensions = [sWidth,sHeight]
        position = newModel2window
        focalPoint = centerPointStart
        if not too_big and not too_small:
            self.Scale(dimensions,position,focalPoint)
        else:
            if sWidth != CurrentWidth:
                if too_small:
                    self.parent.Parent.minZoom = self.parent.Parent.zoomLevel
                    if sWidth < self.minWidth:
                        dimensions[0] = self.minWidth
                    else:
                        dimensions[1] = self.minHeight
                    self.Scale(dimensions,position,focalPoint)
                elif too_big:
                    self.parent.Parent.maxZoom = self.parent.Parent.zoomLevel
                    if sWidth > self.maxWidth:
                        dimensions[0] = self.maxWidth
                    else:
                        dimensions[1] = self.maxHeight
                    self.Scale(dimensions,position,focalPoint)
                    
    def Scale(self,dimensions,position,focalPoint):
        '''
        Applies scale & translate to image
        '''
        self.parent.Freeze()
        self.data2 = self.data.Copy()
        try:
            image = self.data.Scale(dimensions[0],dimensions[1],quality=wx.IMAGE_QUALITY_NORMAL)
        except:
            image = self.data2
        self.bmp = wx.BitmapFromImage(image)
        self.display.SetBitmap(self.bmp)
        
        self.parent.Parent.MX = position[0]
        self.parent.Parent.MY = position[1]
        self.display.Move(position)
        self.marker.Move(focalPoint)
    ...
    def Scale_OLD(self,dimensions,position,focalPoint):
        '''
        Applies scale & translate to image
        '''
        #try:
...
#            print"Mousewheel"
        if event.GetWheelRotation()!= 0:
            self.PostZoom(event.GetWheelRotation(),event)
        ...
        #self.thaw()
            #self.updateMyInfo()
    def PostZoom(self,amount,event):
        '''
        ...
        too_small = (self.zoomLevel + factor) < self.minZoom
        too_big = (self.zoomLevel + factor) > self.maxZoom
        if not too_big and not too_small:
            ...
            if not self.Zooming:
                ...
                mouseCenter = [self.newMx, self.newMy]
                thread.start_new_thread(self.Model.Zoom,(self.zoomLevel,mouseCenter))
            else:
                self.Zooms.append(self.zoomLevel)
        else:
            print"Too big or too small"
        print"Zoom:", self.zoomLevel
