...
def chopImage(path='world.jpg',col_row=(False,False),border = False):
	...
	return images#,sprites,batch#, subHeight,subWidth, x,y

def visible(image,coords,winOffset=False):
	'''
...
def makeBatch(images):
	...
	for image,coords in images:
		if visible(image,coords):
			x,y = coords
			sprites.append(sprites.append(pyglet.sprite.Sprite(image,x=x,y=y,batch=batch)))
